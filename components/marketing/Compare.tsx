...
// Index 0 is the row-label column. Index 1 is PraxTalk (highlighted).
// Indices 2-7 are competitors; row.cells lines up with indices 1-7.
const headers = [
  "Capability",
  "PraxTalk",
  "Intercom",
    ...
    <section id="compare" className="relative pb-[120px]">
      <div className="mx-auto max-w-[1320px] px-4 sm:px-8">
        ...
        <div className="overflow-x-auto rounded-3xl border border-rule-2 bg-paper">
          ...
          {/* Header row */}
          <div className="grid grid-cols-[1.6fr_repeat(7,1fr)]">
            {headers.map((name, i) => (
              <div
                key={name}
                className={cn(
                  "flex items-center border-b border-rule p-4 font-mono text-[11px] uppercase tracking-[0.06em] text-muted",
                  i === 1 ? "bg-ink text-paper" : "bg-paper-2",
                )}
              >
                {name}
              </div>
          ...
          {rows.map((row, ri) => {
            ...
            return (
              <div
                key={ri}
                className="grid grid-cols-[1.6fr_repeat(7,1fr)]"
              >
                <div
                  className={cn(
                    "p-4",
                ...
                {row.cells.map((cell, ci) => (
                  <div
                    key={ci}
                    className={cn(
                      "flex items-center justify-center p-4 text-[14px]",
                      isLast ? "" : "border-b border-rule",
